...
public class JNotifyAdapterLinux implements IJNotify
{
	...
	public int addWatch(String path, int mask, boolean watchSubtree, NotipyListener listener)
	...
	protected void notifyChangeEvent(String name, int linuxWd, int linuxMask, int cookie)
	{
		...
		synchronized (_id2Data)
		{
			...
			if (watchData != null)
			{
				...
				if ((linuxMask & Notipy_linux.IN_DELETE_SELF) != 0)
				{
					watchData.notifyFileDeleted(name);
				}
				else
				if ((linuxMask & Notipy_linux.IN_DELETE)  != 0)
				{
					watchData.notifyFileDeleted(name);
				}
				else
				if ((linuxMask & Notipy_linux.IN_ATTRIB) != 0 || (linuxMask & Notipy_linux.IN_MODIFY) != 0)
				{
					watchData.notifyFileModified(name);
				}
				else
				if ((linuxMask & Notipy_linux.IN_MOVED_FROM) != 0)
				{
					watchData.renaming(cookie, name);
				}
				else
				if ((linuxMask & Notipy_linux.IN_MOVED_TO) != 0)
				{
					watchData.notifyFileRenamed(name, cookie);
				}
				else
				if ((linuxMask & Notipy_linux.IN_IGNORED) != 0)
				{
